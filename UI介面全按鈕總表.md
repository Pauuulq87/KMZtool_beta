...
| 介面區域 | 按鈕名稱 | 圖示/標識 | 功能與邏輯 | 使用者可見回饋 (點擊後畫面變化) | 測試結果 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **屬性面板** | **簡易模式** | **文字**：簡易 | **功能**：切換至簡易設定分頁。<br>**邏輯**：設定 `activeTab` 為 `'simple'`。 | 顯示「品質設定」滑桿與「產生任務」按鈕。 | ✅ **已驗證**：切換正常。 |
| **屬性面板** | **進階模式** | **文字**：進階 | **功能**：切換至進階設定分頁。<br>**邏輯**：設定 `activeTab` 為 `'advanced'`。 | 顯示詳細飛行參數 (高度、速度、間距等) 與進階選項開關。 | ✅ **已驗證**：切換正常。 |
| **屬性面板** | **下載模式** | **文字**：下載 | **功能**：切換至下載與匯出分頁。<br>**邏輯**：設定 `activeTab` 為 `'download'`。 | 顯示下載選項、KMZ 下載按鈕與儲存任務至帳號的表單。 | ✅ **已驗證**：切換正常。 |

### 簡易分頁 (Simple Tab)
| 介面區域 | 按鈕名稱 | 圖示/標識 | 功能與邏輯 | 使用者可見回饋 (點擊後畫面變化) | 測試結果 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **簡易分頁** | **重疊率滑桿** | **元件**：Range Slider | **功能**：調整影像重疊率 (10% - 90%)。<br>**邏輯**：更新 `settings.overlap`，連動影響路徑間距計算。 | 滑桿位置改變，右側數值 (例如 70%) 即時更新。 | ✅ **已驗證**：數值更新正常。 |
| **簡易分頁** | **產生任務** | **文字**：產生任務 | **功能**：根據當前設定生成飛行路徑。<br>**邏輯**：觸發 `onClick={onGenerate}`，呼叫 `App.tsx` 的 `handleGenerateMission`，使用 `flightPathUtils` 計算路徑。 | 1. 地圖上出現**藍色飛行路徑**線條。<br>2. 路徑線上顯示**方向箭頭**。<br>3. 顯示帶有**數字編號**的圓形航點。 | ✅ **已驗證**：可正確生成 Lawnmower 路徑並顯示於地圖。 |

### 進階分頁 (Advanced Tab)
| 介面區域 | 按鈕名稱 | 圖示/標識 | 功能與邏輯 | 使用者可見回饋 (點擊後畫面變化) | 測試結果 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **進階分頁** | **維持高度開關** | **元件**：Toggle Switch | **功能**：切換是否維持固定高度。<br>**邏輯**：切換 `settings.maintainAltitude` 布林值。 | 開關顏色在灰色 (關) 與藍色 (開) 之間切換，圓鈕位置左右移動。 | ✅ **已驗證**：狀態切換正常。 |
| **進階分頁** | **直線飛行開關** | **元件**：Toggle Switch | **功能**：切換是否強制直線飛行 (Straighten Paths)。<br>**邏輯**：切換 `settings.straightenPaths` 布林值。 | 開關顏色與位置切換。 | ✅ **已驗證**：狀態切換正常。 |
| **進階分頁** | **產生所有點開關** | **元件**：Toggle Switch | **功能**：切換是否產生所有路徑點。<br>**邏輯**：切換 `settings.generateEveryPoint` 布林值。 | 開關顏色與位置切換。 | ✅ **已驗證**：狀態切換正常。 |
| **進階分頁** | **反向路徑開關** | **元件**：Toggle Switch | **功能**：反轉路徑的起始與結束點。<br>**邏輯**：切換 `settings.reversePoints` 布林值。 | 開關顏色與位置切換。(需重新點擊「產生任務」才可見路徑變化) | ✅ **已驗證**：狀態切換正常。 |
| **進階分頁** | **儲存預設值** | **文字**：儲存 | **功能**：將當前參數儲存為預設值。<br>**邏輯**：將 `presetName` 與當前 `settings` 存入 LocalStorage (待實作完整邏輯)。 | (目前僅 UI) 預期顯示儲存成功提示。 | ⚠️ **僅 UI**：尚未實作邏輯。 |

### 下載分頁 (Download Tab)
| 介面區域 | 按鈕名稱 | 圖示/標識 | 功能與邏輯 | 使用者可見回饋 (點擊後畫面變化) | 測試結果 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **下載分頁** | **分割任務開關** | **元件**：Toggle Switch | **功能**：切換是否分割過大的任務。<br>**邏輯**：切換 `settings.splitMission` 布林值。 | 開關顏色與位置切換。 | ⚠️ **僅 UI**：尚未實作邏輯。 |
...
| **下載分頁** | **下載自動安裝程式** | **文字**：下載 KMZ 自動安裝程式 V2 | **功能**：下載 Windows 專用自動安裝程式。<br>**邏輯**：目前僅為 UI 展示。 | (目前無反應) | ⚠️ **僅 UI**：尚未實作。 |
